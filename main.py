...
import numpy as np
from transformers import VideoMAEConfig, Wav2Vec2Config, Wav2Vec2Tokenizer, AutoTokenizer, AutoModelForCausalLM
import os
import cv2
import numpy as np
...
def ctc_decode_phonemes(predicted_indices, reverse_vocab, blank_idx=0):
    ...
    Args:
        predicted_indices: List of phoneme indices
        reverse_vocab: Dict mapping indices to phonemes
        blank_idx: Index of the blank/pad token (default 0)
    Returns:
        List of decoded phoneme strings
    """
    decoded = []
    prev_idx = None
...
def load_videos(video_path, num_frames=16, frame_size=(224, 224), use_face_cropping=True):
    """
    Load a video and extract frames with optional face detection.
    Args:
        video_path: Path to the video file.
        num_frames: Number of frames to extract.
        frame_size: Desired size for frames (height, width).
        use_face_cropping: Whether to detect and crop faces.
    Returns:
        Tensor containing video frames or None if loading fails.
    """
    ...
    if frame_count < num_frames:
        print(f"Warning: Not enough frames in video {video_path}. Using available {frame_count} frames.")
    ...
    # Initialize face cropper if needed
    face_cropper = None
    if use_face_cropping:
        try:
            ...
            print(f"Warning: Could not initialize face cropper: {e}. Using full frames.")
            face_cropper = None
    ...
    for idx in sample_indices:
        ...
        # Apply face detection and cropping if enabled
    ...
    if face_cropper is not None:
        print(f"✓ Face detected in {face_detected_count}/{len(frames)} frames")
...
def phonemes_to_text_llm(phoneme_sequence, llm_model_path=None, device='cuda'):
    ...
    Args:
        phoneme_sequence: List of phoneme strings (e.g., ['DH', 'AH', 'K', 'AE', 'T'])
        llm_model_path: Path to fine-tuned LLM model (optional)
        device: Device to run inference on
    Returns:
        Generated text string
    """
    if llm_model_path is None or not os.path.exists(llm_model_path):
        print("\nStage 2 (LLM) not available. Returning phoneme sequence only.")
        return " ".join(phoneme_sequence)
    
    try:
        print(f"\nLoading Stage 2 LLM from {llm_model_path}...")
        ...
        # Format phonemes as input (matching training format)
        phoneme_str = " ".join(phoneme_sequence)
        prompt = f"<S2S>\n<PHONEMES>\n{phoneme_str}\n</PHONEMES>\n<TEXT>\n"
        
        print(f"Stage 2 input: {phoneme_str}")
        ...
        # Extract text after <TEXT> tag
        if "<TEXT>" in generated_text:
            text_output = generated_text.split("<TEXT>")[-1].strip()
            # Remove any remaining special tokens
            for tag in ["<S2S>", "<PHONEMES>", "</PHONEMES>", "<TEXT>"]:
        ...
        print(f"Error in Stage 2 LLM inference: {e}")
        return " ".join(phoneme_sequence)

def run_inference(model_path, version, video_path, device, phoneme_vocab, llm_model_path=None, beam_width=3):
    """
    Run two-stage inference: Video -> Phonemes -> Text
    Args:
        model_path: Path to the saved Stage 1 model (video-to-phoneme).
        version: Model version ('V1' or 'V2')
        video_path: Path to the video file.
        device: Device to run the inference on (CPU or GPU).
        phoneme_vocab: Phoneme vocabulary dictionary
        llm_model_path: Path to Stage 2 LLM model (optional)
        beam_width: Width of the beam for beam search decoding.
    Returns:
        Dictionary with phonemes, text, and raw logits
    """
    print(f"\n{'='*60}")
    print("VALLR Two-Stage Inference")
    print(f"{'='*60}")
    ...
    # Load the video inputs with face detection
    video_inputs = load_videos(video_path, num_frames=16, use_face_cropping=True)

    if video_inputs is None:
        print("Error: Failed to load video frames.")
        return {"error": "Failed to load video", "phonemes": [], "text": "", "logits": None}

    print(f"Video loaded: shape={video_inputs.shape}")
    ...
    with torch.no_grad():
        ...
        for pred_idx_seq in predicted_indices:
            # Raw phonemes (with pads)
            raw_phonemes = [reverse_vocab.get(idx.item(), '<unk>') for idx in pred_idx_seq]
            raw_phonemes_list.append(raw_phonemes)
            
            # CTC decoded phonemes (cleaned)
            decoded_phonemes = ctc_decode_phonemes(
                pred_idx_seq.tolist(), 
                reverse_vocab, 
                blank_idx=phoneme_vocab.get('<pad>', 0)
            )
            decoded_phonemes_list.append(decoded_phonemes)

    print(f"\nRaw phonemes (with padding): {raw_phonemes_list[0][:20]}...")  # Show first 20
    print(f"Decoded phonemes (CTC): {decoded_phonemes_list[0]}")
    
    # STAGE 2: Phonemes -> Text (if LLM model available)
    print(f"\n[Stage 2] Phonemes to Text")
    final_text = phonemes_to_text_llm(decoded_phonemes_list[0], llm_model_path, device)
    
    print(f"\n{'='*60}")
    print("RESULTS:")
    print(f"{'='*60}")
    print(f"Phonemes: {' '.join(decoded_phonemes_list[0])}")
    print(f"Text: {final_text}")
    print(f"{'='*60}\n")

    return {
        "raw_phonemes": raw_phonemes_list[0],
        "decoded_phonemes": decoded_phonemes_list[0],
        "text": final_text,
        "logits": logits,
        "features": feats
    }
...
def main(args):
    ...
    if mode == "train":
        ...
        # Check if LLM model path is provided via environment variable or config
